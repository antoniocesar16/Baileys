...
interface userInputs {
	...
	code: string // codigo de verificação
...
// otp pairing code
app.get("/otp/:code", (req: Request, res: Response) => {
...
// registration method
app.get("/register/:type", (req: Request, res: Response) => {
	const type = req.params.type;
	user.registerType = type;

	res.send("");
});

app.listen(port, () => 'server running on port 8001')
...
const connectPhone = async(phone: string) => {
	...
	const sock = makeWASocket({
		version,
		logger,
		printQRInTerminal: true, // !usePairingCode, // print QR in terminal
		mobile: false,
		auth: {
			creds: state.creds,
	...
	store?.bind(sock.ev)
		...
		if(useMobile && !sock.authState.creds.registered) {
			...
			// 	sock.authState.creds.pairingCode = code
			// }

			if(!registration.phoneNumber || !user.phone) {
				console.log('Please enter your mobile phone number:', user.phone)
				registration.phoneNumber = user.phone
			}
	
			const libPhonenumber = await import("libphonenumber-js")
			const phoneNumber = libPhonenumber.parsePhoneNumber(registration!.phoneNumber, { defaultCountry: 'BR' })
			...
			async function askForOTP() {
				...
				try {
					...
					if(error?.reason === 'too_many_tries') {
						console.error('You have requested too many codes. Please try again later.')
						await delay(30000)
						await askForOTP()
					}

					if(error?.reason === 'old_version') {
						console.error('You are using an old version of WhatsApp. Please update your app.')
						await delay(30000)
						await askForOTP()
					}
